...
<head>
    ...
    <title>{{ $general->sitename(__($pageTitle ?? 'Admin Auth')) }}</title>
    <script>
        (function () {
            try {
            ...
            <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                <a href="{{ url('/') }}" class="flex items-center gap-3">
                    ...
                    <div class="leading-tight">
                        ...
                        <p class="text-sm text-slate-500 dark:text-zinc-400">Admin access</p>
                    </div>
                </a>
                ...
                <section class="hidden rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_30px_90px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-zinc-900/70 lg:flex lg:flex-col lg:justify-between">
                    <div class="space-y-8">
                        ...
                        <div class="space-y-4">
                            <h1 class="max-w-xl text-4xl font-semibold tracking-tight text-slate-950 dark:text-white">
                                Manage users, payments, and reports from one calm dashboard.
                            </h1>
                            <p class="max-w-xl text-base leading-7 text-slate-600 dark:text-zinc-300">
                                The admin frame now shares the same white-first Flux-inspired surface as the user app, so the whole product feels consistent.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                        <div class="rounded-3xl border border-slate-200 p-4 dark:border-white/10">
                            <p class="text-sm font-medium text-slate-500 dark:text-zinc-400">Access</p>
                            <p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">Role-based</p>
                        </div>
                        <div class="rounded-3xl border border-slate-200 p-4 dark:border-white/10">
                            <p class="text-sm font-medium text-slate-500 dark:text-zinc-400">Payments</p>
                            <p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">Audited</p>
                        </div>
                        <div class="rounded-3xl border border-slate-200 p-4 dark:border-white/10">
                            <p class="text-sm font-medium text-slate-500 dark:text-zinc-400">Theme</p>
                            <p class="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">Persistent</p>
                        </div>
                    </div>
                </section>
