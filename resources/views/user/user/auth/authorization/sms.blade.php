...
@section('content')
    <div class="space-y-6">
        <div class="space-y-2">
            ...
            <h1 class="text-3xl font-semibold tracking-tight text-slate-950 dark:text-white">@lang('Verify your phone number')</h1>
            <p class="text-sm leading-6 text-slate-600 dark:text-zinc-300">
                @lang('Enter the SMS code sent to your phone number to continue to your account.')
            </p>
        </div>

        <form method="POST" action="{{ route('user.verify.sms') }}" class="space-y-5" data-busy-form data-busy-message="Verifying your SMS code...">
            @csrf

            <div class="space-y-2">
                <label for="sms_verified_code" class="text-sm font-medium text-slate-700 dark:text-zinc-200">@lang('Verification code')</label>
                <input
                    type="text"
                    name="sms_verified_code"
                    id="sms_verified_code"
                    inputmode="numeric"
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/5">
            <p class="text-sm text-slate-600 dark:text-zinc-400">@lang('Need another SMS code?')</p>
            <a href="{{ route('user.send.verify.code', ['type' => 'phone']) }}" class="inline-flex h-10 items-center justify-center rounded-full border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:border-sky-200 hover:text-sky-700 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-200 dark:hover:border-sky-500/30 dark:hover:text-sky-300">
                @lang('Resend code')
            </a>
        </div>
    </div>
