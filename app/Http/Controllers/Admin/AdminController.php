...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $payment['total_deposit_charge_monnify'] = Deposit::where('status',1)->where('method_code',104)->sum('charge');
        $payment['total_deposit_charge_paystack'] = Deposit::where('status',1)->where('method_code',107)->sum('charge');
        $payment['total_deposit_pending'] = Deposit::where('status',1)->count();
        $payment['total_purchase'] = Bill::where('status',1)->sum('amount');
        $payment['total_deposit_monnify'] = Deposit::where('method_code',104)->where('status', 1)->sum('amount');
        $payment['total_deposit_paystack'] = Deposit::where('method_code',107)->where('status', 1)->sum('amount');
        $payment['total_internet_purchase'] = Bill::where('status',1)->where('type',2)->count();
        $payment['total_internet_amount'] = Bill::where('status',1)->where('type',2)->sum('amount');
        $payment['total_users_balance'] = User::where('status',1)->sum('balance');
        $payment['total_internet_profit'] = Bill::where('status',1)->where('type',2)->sum('profit');
        $payment['total_airtime_purchase'] = Bill::where('status',1)->where('type',1)->count();
        $payment['total_purchase_success'] = Bill::where('status',1)->count();
        $payment['total_purchase_failed'] = Bill::where('status',0)->count();
        $payment['total_airtime_amount'] = Bill::where('status',1)->where('type',1)->sum('amount');
        $payment['total_airtime_profit'] = Bill::where('status',1)->where('type',1)->sum('profit');
        $payment['total_monnify_purchase'] = Deposit::where('status',1)->where('method_code',104)->count();
        $payment['total_estim_profit'] = Deposit::where('status',1)->sum('charge');
